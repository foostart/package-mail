...
class Mail extends FooModel {
            ...
            foreach($params as $column => $value)
            {
                if($this->isValidValue($value))
                {
                    switch($column)
                    {
                        case 'mail_name':
                            if (!empty($value)) {
                                $elo = $elo->where($this->table . '.mail_name', '=', $value);
                            }
                            break;
                        case 'status':
                            if (!empty($value)) {
                        ...
                        case 'keyword':
                            if (!empty($value)) {
                                $elo = $elo->where(function($elo) use ($value) {
                                    $elo->where($this->table . '.mail_name', 'LIKE', "%{$value}%")
                                    ->orWhere($this->table . '.mail_description','LIKE', "%{$value}%")
                                    ->orWhere($this->table . '.mail_overview','LIKE', "%{$value}%");
                                });
                            }
                            break;
                        default:
    ...
    public function updateItem($params = [], $id = NULL) {
        ...
        if (!empty($mail)) {
            ...
            $mail->$field_status = $this->status['publish'];
