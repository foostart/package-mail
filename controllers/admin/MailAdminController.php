...
use Foostart\Mail\Validators\MailValidator;


class MailAdminController extends FooController {
    ...
    public function __construct() {
        ...
        parent::__construct();
        // models
        $this->obj_item = new Mail(array('perPage' => 10));
        $this->obj_category = new Category();

        // validators
        $this->obj_validator = new MailValidator();

        // set language files
        $this->plang_admin = 'mail-admin';
        $this->plang_front = 'mail-front';

        // package name
        $this->package_name = 'package-mail';
        $this->package_base_name = 'mail';

        // root routers
        $this->root_router = 'mails';
        ...
        // //set category
        $this->category_ref_name = 'admin/mails';
    ...
    public function config(Request $request) {
        $is_valid_request = $this->isValidRequest($request);
        // display view
        $config_path = realpath(base_path('config/package-mail.php'));
        $package_path = realpath(base_path('vendor/foostart/package-mail'));
        ...
        if ($request->isMethod('post') && $is_valid_request) {
            ...
            file_put_contents($config_bakup.'/package-mail-'.date('YmdHis',time()).'.php', $content);
    ...
    public function lang(Request $request) {
        $is_valid_request = $this->isValidRequest($request);
        // display view
        $langs = config('package-mail.langs');
        $lang_paths = [];

        if (!empty($langs) && is_array($langs)) {
            foreach ($langs as $key => $value) {
                $lang_paths[$key] = realpath(base_path('resources/lang/'.$key.'/mail-admin.php'));
            }
        }
        ...
        if ($request->isMethod('post') && $is_valid_request) {
            ...
            foreach ($lang_paths as $key => $value) {
                ...
                //format file name mail-admin-YmdHis.php
                file_put_contents($lang_bakup.'/'.$key.'/mail-admin-'.date('YmdHis',time()).'.php', $content);
            }
